...
\noindent\textbf{RQ1 (Effectiveness):} Yes. On the within-CIFAR \emph{airplane-vs-rest} split (a single ID class; multi-class generalisation is future work), the final selected $\lambda{=}0.02$ model reaches $99.03\% \pm 0.07\%$ AUROC over three seeds at $K{=}50$. The reproducible seed-42 $K{=}100$ evaluation reaches $98.98\%$ AUROC and $90.50$--$96.97\%$ across five reproducible external OOD datasets, evaluated against the full CIFAR-10 test reference pool used in the external protocol (Section~\ref{sec:results:cifar}), confirming reconstruction error as a robust distributional-fit signal within this scope.
...
\noindent\textbf{RQ3 (Monte Carlo Trials):} Performance rises sharply to $K=10$ ($98.2\%$ AUROC at $5\times$ speedup over $K=50$) then saturates (Section~\ref{sec:results:ablations}). This design ablation used the seed-42 $\lambda=0.01$ checkpoint for compute-budget reasons, while the final selected model uses $\lambda=0.02$. $K=10$ is recommended for most deployments; $K=1$ ($91.0\%$) serves as a fast fallback. Transferability of the $K$--AUROC curve to $\lambda=0.02$ was not independently verified.

\noindent\textbf{RQ4 (Scoring Strategy):} Contrastive scoring is essential; in the seed-42 $\lambda=0.01$ scoring ablation, ID-error-only scoring collapses on SVHN ($20.2\%$ AUROC). Difference scoring is retained as the default for its lower FPR95 and simpler interpretation (Section~\ref{sec:results:ablations}).

\noindent\textbf{RQ5 (Practical Viability):} Inference cost is higher than discriminative baselines but manageable: $K=10$ requires ${\sim}16$~min per 10K images on an NVIDIA GeForce RTX 2080 Ti. The accuracy--cost trade-off is favourable when the cost of a missed OOD sample exceeds the inference overhead.
...
\noindent\textbf{Empirical Contribution:} Comprehensive evaluation across two tracks (CIFAR-10 OOD detection in Chapters~\ref{chap:results}--\ref{chap:discussion}, and public inkjet CDM quality classification under 5-fold cross-validation), producing reproducible results and clearly scoped ablation studies.

\noindent\textbf{Practical and Implementation Contribution:} Actionable deployment guidelines (separation loss for large-scale OOD; explicit public reproduction paths for industrial CDM evaluation) and modular public implementations built with PyTorch Lightning and Hydra. The CIFAR-10/OOD track is available at \url{https://github.com/ahmed-3m/DiffusionOOD} with weights at \url{https://huggingface.co/ahmed-3m/DiffusionOOD}; the inkjet CDM track is available at \url{https://github.com/ahmed-3m/InkjetOOD} with weights at \url{https://huggingface.co/ahmed-3m/InkjetOOD}, and uses the public FTI\_Zer0P dataset~\citep{profactor2024fti}.
