...
%% ------------------------------------------------------------------
\subsection{The Separation Loss Dominates the CIFAR-10 Ablation}
...
$\lambda = 0.001$ already yields 97.32\%, the best-tested setting is
...
%% ------------------------------------------------------------------
\subsection{External OOD Generalisation Reveals Learnt ID Manifold}
\label{sec:disc:cifar:external}

Across five reproducible external benchmarks, AUROC ranges from 90.50\% to
...
overlap with certain airplane image regions.
...
Uniform sampling across $[1, T]$ is the best-tested strategy. The per-timestep analysis
...
%% ------------------------------------------------------------------
\subsection{Contrastive Scoring Transfers Across the Two Tracks}
\label{sec:disc:cross:contrastive}

Both evaluated tracks support the use of scores that compare
...
reproducible results are consistent with a support-based interpretation,
...
about 16.2\,min per 10K images on an NVIDIA GeForce RTX 2080 Ti;
$K = 50$: about 81.0\,min), and the task has sufficient labelled data
...
settings. In the inkjet comparison, the ResNet baselines use
$224{\times}224$ inputs with ImageNet pre-training whereas the CDM operates on
YOLO-cropped $128{\times}128$ feature images; absolute comparisons are
therefore affected by both model family and input-resolution/context
differences. For
...
Even at $K{=}10$, the binary CDM is $68.6{\times}$ slower than the
measured discriminative ResNet-18 forward pass on the same RTX 2080 Ti,
precluding real-time applications.
...
as a task-dependent hyperparameter selected via ablation.

\paragraph{Threshold calibration.}
Although the CDM is not trained on external OOD data---CIFAR model
selection and early stopping use only the within-CIFAR validation split
(Section~\ref{sec:exp:protocol})---practical deployment still requires
choosing an operating threshold. The benchmark operating points reported
here are evaluated on retained test artefacts; deployment to a new domain
would require representative OOD or defective validation samples for
recalibration, or otherwise conservative thresholding. This distinction
is deliberate: no external OOD data leaks into model selection, but
threshold setting remains a separate, deployment-time calibration step.
...
PAC-style bounds). Informal Claim~2 in
Section~\ref{sec:meth:diffusion:formal}
