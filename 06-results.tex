...
% Stage-2 aggressive defaults for the chapter: avoids orphan section
% headings, single-float half-page whitespace, and the table+figure
% page bleed pattern. Individual float-heavy pages can opt out locally
% (see e.g. Figure 6.1 + Table 6.1 below, wrapped in their own \begingroup).
...
% --- Local override: airier spacing for Figure 6.1 + Table 6.1 page ---
% This page is float-only (one figure + one table, no prose) and looks
% cramped under the chapter's aggressive defaults. Restore default-like
% values around just this pair so the page matches the airier original
% layout. The outer chapter \begingroup is unaffected.
\begingroup
...
\end{table}
\endgroup
% --- End of local override for Figure 6.1 + Table 6.1 ---
