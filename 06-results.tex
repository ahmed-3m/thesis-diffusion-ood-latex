...
% --- Local Chapter-6 float-spacing tightening (reversible, scoped) ---
% Moderate shave: enough to avoid orphan section headings and the
% "single float + half-page whitespace" pattern, but retains visual
% breathing room on float-heavy pages (e.g. p52: Figure 6.1 + Table 6.1).
% Closed by the matching \endgroup at end of file.
\begingroup
% Glue around floats — moderate values (between defaults and Stage-2 aggressive)
\setlength{\textfloatsep}{14pt plus 2pt minus 2pt}    % was 20pt default
\setlength{\floatsep}{10pt plus 2pt minus 2pt}        % was 12pt default
\setlength{\intextsep}{10pt plus 2pt minus 2pt}       % was 12pt default
\setlength{\abovecaptionskip}{6pt}                     % was 10pt default
\setlength{\belowcaptionskip}{2pt}
\makeatletter
% Restore moderate elastic at top of float-only pages so figures don't
% glue to the page chrome; keep more elastic at the bottom so whitespace
% accumulates predictably at the page foot.
\setlength{\@fptop}{0pt plus 0.3fil}
\setlength{\@fpsep}{10pt plus 0fil}
\setlength{\@fpbot}{0pt plus 1fil}
\makeatother
% Float-placement fractions (allow more float per page; require less surrounding text)
\renewcommand{\topfraction}{0.90}        % up from 0.70 default
\renewcommand{\bottomfraction}{0.60}     % up from 0.30 default
\renewcommand{\textfraction}{0.10}       % down from 0.20 default
\renewcommand{\floatpagefraction}{0.70}  % up from 0.50 default
\renewcommand{\dbltopfraction}{0.90}
\renewcommand{\dblfloatpagefraction}{0.70}
\setcounter{topnumber}{3}                 % up from 2 default
\setcounter{bottomnumber}{2}              % up from 1 default
\setcounter{totalnumber}{5}               % up from 3 default
...
For the CIFAR-10 track, the two retained multi-seed separation loss
weights ($\lambda = 0.01$, used for several design ablations, and
$\lambda = 0.02$, the final selected weight) were each re-trained over three
independent seeds (42, 123, and 456). The no-separation baseline
($\lambda = 0.0$) was later re-evaluated over the same three seeds in
the final consistency pass.
The seed-stability figure in this chapter refers to the final selected
$\lambda = 0.02$ configuration. The training-summary figure retains the
earlier three-seed $\lambda = 0.01$ checkpoint because that is the
complete reproducible checkpoint series preserved in the submission
artefacts.
...
Table~\ref{tab:results:main-auroc} reports the reproducible core metrics
...
\begin{figure}[htbp]
  ...
  \caption{Validation AUROC across training seeds (42/123/456) from
    ...
    core quantitative claims are taken from reproducible raw-score
...
\begin{table}[htbp]
  \centering
  \caption{Binary CDM OOD detection performance from reproducible
    artefacts. Core claims use only metrics reproducible from current
    raw score files (seed~42, $\lambda{=}0.02$, $K{=}100$, difference scoring). $K{=}100$
    is used here for statistical stability; ablation studies use
    $K{=}50$ (Section~\ref{sec:exp:protocol}).}
  ...
  \begin{tabular}{llccc}
    ...
    Dataset & Type & AUROC (\%)$\uparrow$ & FPR95 (\%)$\downarrow$
      & AUPRC (\%)$\uparrow$ \\
    \midrule
    \multicolumn{5}{l}{\textbf{Core reproducible metrics (current raw scores)}} \\
    \midrule
...
\paragraph{Within-CIFAR Performance (core reproducible).}
Using current raw-score tensors (seed~42, $\lambda=0.02$, $K = 100$), the binary CDM
achieves 98.98\% AUROC with FPR95 of 4.7\% on the airplane-vs-rest
CIFAR-10 split.
Figure~\ref{fig:results:training-summary} provides the reproducible
checkpoint summary for the retained three-seed $\lambda = 0.01$
ablation checkpoint, which is later reused for the $K$, timestep, and
scoring-strategy studies.
...
\begin{figure}[htbp]
  ...
  \caption{Training summary from checkpoint metadata: best validation
    AUROC and best epoch for seeds~42, 123, and 456 for the retained
    $\lambda{=}0.01$ ablation checkpoint.}
...
Under the public-evidence protocol, core external claims use only datasets with
released raw-score tensors.  This yields five reproducible external
benchmarks: CIFAR-100, Places365, FashionMNIST, Textures, and SVHN.

\paragraph{Reproducible external profile.}
AUROC ranges from 90.50\% (SVHN) to 96.97\% (CIFAR-100), with a mean of
94.17\% across the five reproducible external sets, each evaluated against
...
\begin{table}[htbp]
  ...
  \caption{Comparison with one-class novelty detection baselines on the
    ...
    \cite{reiss2021panda}. Our CDM rows use $K{=}50$ and three-seed
    means over seeds 42/123/456 where available. The $\lambda{=}0$ row
    reports the no-separation-loss baseline; the $\lambda{=}0.02$ row
    reports the best-tested separation-loss setting. This comparison is
    asymmetric because the CDM is trained with an OOD-proxy class, whereas
    the one-class baselines are trained only on ID samples.}
  ...
  \begin{tabular}{llcl}
    ...
    \midrule
    Binary CDM ($\lambda{=}0$)   & Generative & $92.52 \pm 11.07$  & Ours \\
    Binary CDM ($\lambda{=}0.02$) & Generative + sep.\ loss & $99.03 \pm 0.07$ & Ours \\
    \bottomrule
  \end{tabular}
\end{table}

Without the separation loss, the binary CDM is competitive on average
but highly seed-sensitive. With $\lambda{=}0.02$, performance becomes
both stronger and substantially more stable, exceeding the listed
one-class baselines in this single-class airplane-vs-rest setting
\emph{under this asymmetric protocol}: our CDM trains with an OOD-proxy
class while the one-class baselines train on ID data only, so the
margin should be read as contextual rather than as a like-for-like
ranking (see caveats below).
...
\begin{table}[htbp]
  ...
  \caption{Effect of number of Monte Carlo trials $K$ on OOD scoring.
    Evaluated on CIFAR-10 binary test set using the seed-42
    $\lambda{=}0.01$ checkpoint.
...
\begin{figure}[htbp]
  ...
  \caption{Effect of $K$ on AUROC (left axis) and inference time for
    10K images (right axis), evaluated with the seed-42
    $\lambda{=}0.01$ checkpoint. The curve flattens after
...
best-tested $\lambda = 0.02$ setting) for compute-budget reasons.
...
but exact numbers at the best-tested weight have not been independently
...
\begin{table}[htbp]
  \centering
  \caption{Comparison of timestep sampling strategies using the seed-42
    $\lambda{=}0.01$ checkpoint and $K{=}50$.}
...
\paragraph{Uniform sampling is the best-tested strategy.}
...
\begin{figure}[htbp]
  ...
  \caption{AUROC for three timestep sampling strategies on the
    within-CIFAR binary split and SVHN ($K = 50$, seed-42,
    $\lambda{=}0.01$). Uniform
...
\begin{table}[htbp]
  \centering
  \caption{Comparison of OOD scoring methods (seed-42 $\lambda{=}0.01$ checkpoint,
...
\begin{figure}[htbp]
  ...
  \caption{AUROC comparison for three scoring methods on Within-CIFAR
    and SVHN ($K = 50$, seed-42, $\lambda{=}0.01$). Difference and ratio scoring both
...
It adds an explicit training signal that pushes apart the
class-conditional noise predictions, indirectly widening the
reconstruction-error gap between ID and OOD-conditioned predictions.
...
\begin{table}[htbp]
  ...
  \caption{Effect of separation loss weight $\lambda$ on OOD detection
    ...
    $\lambda \in \{0.0, 0.01, 0.02\}$ and seed-42 for the remaining
  ...
  \begin{tabular}{lccc}
    \toprule
    $\lambda$ & Within-CIFAR AUROC (\%) & Best Epoch (seed 42)
      & SVHN AUROC (\%) \\
    \midrule
    0.0    & $92.52 \pm 11.07$ &  79 & 100.0\textsuperscript{$\dagger$} \\
    0.001  & 97.32              &  19 & 92.0 \\
    0.01   & $98.82 \pm 0.06$  &  19 & 90.5 \\
...
\begin{figure}[htbp]
  ...
  \caption{Effect of separation loss weight $\lambda$ on Within-CIFAR
    and SVHN AUROC (left) together with the seed-42 best epoch for each
    retained checkpoint (right), generated from the final consistency
    ablation summary. Crosses in the SVHN series mark documented
    artefact points.}
...
\paragraph{Best-tested at $\lambda = 0.02$ (three-seed).}
...
Inspection of the reproducible raw-score tensors shows a consistent
...
Table~\ref{tab:results:main-auroc}. The retained figure set is regenerated
only from the public raw-score artefacts listed in the reproducibility appendix.
...
\begin{figure}[htbp]
  ...
  \caption{Mean reconstruction error as a function of timestep $t$ from
    the reproducible \texttt{per\_timestep} JSON block. Curves are shown
...
%% ------------------------------------------------------------------
\paragraph{Calibration and decision boundary.}
Figure~\ref{fig:results:calibration} shows the within-CIFAR OOD score distribution for ID and OOD samples at the operating threshold calibrated to TPR $= 95\%$.
The separation between the two score distributions is clear: ID samples (airplane) cluster around negative or near-zero scores (i.e., better reconstruction under the ID condition), while OOD samples accumulate at higher positive values, consistent with the per-timestep error analysis in Figure~\ref{fig:results:per-timestep-error}, which shows that OOD samples incur higher reconstruction error under both class conditions.
The figure confirms that the $s(x) > \tau$ threshold at TPR $= 95\%$ corresponds to the FPR95 values reported in Table~\ref{tab:results:main-auroc}.
...
\begin{figure}[htbp]
  ...
  \caption{Cross-domain effect of separation loss on CDM AUROC. The
    CIFAR-10 panel compares the three-seed $\lambda=0$ baseline with the
    best-tested $\lambda=0.02$ setting; the inkjet panel shows 5-fold CV
    means across $\lambda$ values, all within the baseline fold
    variability.}
...
improvement.
With only 573 processed source-image groups (6,408 feature crops), the
cross-fold standard deviation ($\pm 0.023$) exceeds the observed
single-split gain,
...
\begin{table}[htbp]
  ...
  \caption{Cross-domain comparison of separation loss effect. CIFAR-10
    uses three-seed Within-CIFAR AUROC from the final consistency
    separation-loss summary; Inkjet uses 5-fold CV mean $\pm$ std.}
  ...
  \begin{tabular}{lccc}
    ...
    CIFAR-10   & $0.925 \pm 0.111$
      & $0.9903 \pm 0.0007$ ($\lambda = 0.02$) & +6.5\,pp \\
...
The retained public inkjet pipeline uses 573 processed source-image
groups (6,408 feature crops) versus CIFAR-10's 50,000 images.
...
technique for the evaluated CIFAR-10 binary OOD setting but requires
validation on small domain-specific datasets before deployment.
\endgroup
\endgroup
% --- End of Chapter-6 float-spacing tightening ---
