...
\begin{table}[htbp]
  ...
  \caption{Binary CDM OOD detection performance from reproducible
    ...
    $K{=}50$ (Section~\ref{sec:exp:protocol}). Archived-only datasets are
    excluded from this table and from core claims.}
...
\paragraph{Archived-only datasets.}
Food-101 and STL-10 values from earlier archived JSON files are excluded
from the main table and from all core claims because corresponding
current raw-score tensors are not available in the submission package.
They can be reinstated only by rerunning the public evaluation pipeline
and preserving the resulting raw scores.
...
Table~\ref{tab:results:main-auroc}. Because an older composite figure
bundled obsolete Food-101/STL-10 panels, it is intentionally omitted
from the final thesis build.
