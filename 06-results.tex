...
% --- Local Chapter-6 float-spacing setup (reversible, scoped) ---
% Use the same airier float-page setup as the Figure 6.1 + Table 6.1
% page, so Chapter 6 figure/table pages remain visually consistent.
...
Using current raw-score tensors (seed~42, $\lambda=0.02$, $K = 100$),
the binary CDM achieves 98.98\% AUROC with 4.7\% FPR95 on the
airplane-vs-rest split; Figure~\ref{fig:results:training-summary}
summarises the retained three-seed $\lambda = 0.01$ checkpoint reused
for $K$, timestep, and scoring ablations.
...
separation loss relative to Section~\ref{sec:meth:training:loss}. Three
factors likely explain the contrast with CIFAR-10: (i) the retained
public inkjet pipeline uses only 573 processed source-image groups
(6,408 feature crops) versus CIFAR-10's 50,000 images, making the
separation gradient noisier; (ii) GOOD and BAD inkjet prints differ in
fine defect details rather than a large semantic shift, demanding higher
reconstruction precision; and (iii) the CDM is trained from scratch on a
narrow domain with limited diversity.
Thus, the separation loss is strong for the evaluated CIFAR-10 binary
OOD setting but requires validation on small domain-specific datasets
before deployment.
\endgroup
\endgroup
% --- End of Chapter-6 float-spacing setup ---
