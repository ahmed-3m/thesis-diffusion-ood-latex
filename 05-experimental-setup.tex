...
CIFAR-10~\citep{krizhevsky2009learning} consists of 60,000
...
We evaluate on five public external OOD datasets spanning near- to far-OOD
distributional distances from CIFAR-10.

\textbf{Near-OOD: CIFAR-100}~\citep{krizhevsky2009learning} (10,000 test images,
$32 \times 32$, 100 classes sharing CIFAR-10 image statistics).

\textbf{Far-OOD:} SVHN~\citep{netzer2011reading} (26,032 digit images,
$32 \times 32$); \textbf{FashionMNIST}~\citep{xiao2017fashion} (10,000 greyscale
clothing images, resized to $32 \times 32$, converted to 3-channel);
\textbf{Textures (DTD)}~\citep{cimpoi2014describing} (1,880 texture images,
cropped and resized to $32 \times 32$);
\textbf{Places365}~\citep{zhou2018places} (10,000 scene images,
...
($\{42, 123, 456\}$) for the primary reported separation-loss settings.
The final selected model uses $\lambda = 0.02$; $\lambda = 0.01$ is
retained for the $K$, timestep-sampling, and scoring-strategy ablations
for compute-budget reasons. The three seeds control model
initialisation, data shuffling, and all stochastic training operations.
...
Within-CIFAR AUROC and FPR@95\%TPR are the primary metrics. For
reproducibility, all reported core external-OOD claims use public
datasets, released code, and preserved raw-score artefacts (seed-42
raw-score evaluation; $K = 100$ MC trials).
...
ablation checkpoint $\lambda = 0.01$ is evaluated with three seeds
(seeds 42/123/456); the final selected weight $\lambda = 0.02$ is also
evaluated with three seeds.
...
images on an NVIDIA GeForce RTX 2080 Ti for the runtime table).
...
FPR@95\%TPR are reported for the retained reproducible raw-score runs. Importantly, no
...
within-CIFAR validation split (Section~\ref{sec:exp:protocol:within-cifar}).
...
model weights for the retained seed and $\lambda$ configurations are hosted at
...
from the public \url{https://github.com/ahmed-3m/InkjetOOD}
repository~\cite{mohammed2026inkjetood} at commit \texttt{7c2451c};
the corresponding pre-trained inkjet CDM weights are available at
\url{https://huggingface.co/ahmed-3m/InkjetOOD}. The inkjet track also
requires the public PROFACTOR FTI\_Zer0P dataset on
Zenodo~\citep{profactor2024fti}. After downloading the dataset, the
...
dataset~\citep{profactor2024fti} contains 1,204 inkjet print images
...
crop via differential reconstruction error under GOOD and BAD conditions.
Only results regenerable from \texttt{InkjetOOD}, the public FTI\_Zer0P
dataset, and the released CDM weights are reported.

\subsection{Evaluation Protocol}
\label{sec:exp:inkjet:evaluation}

\paragraph{Cross-validation.}
Stratified 5-fold cross-validation with image-level splitting across 573
source-image groups (random seed 42, fixed across all $\lambda$ settings
so the same fold partitions are evaluated for every condition); all
crops from the same source image stay in the same fold, preventing
leakage. Separation-loss ablation trains the CDM with
$\lambda \in \{0.0, 0.01, 0.02, 0.05\}$ to evaluate whether the
separation loss improves quality classification.

\paragraph{Metrics.}
Primary: AUROC (mean $\pm$ std across 5 folds); secondary: Accuracy, F1,
Precision, Recall. Paired $t$-tests and Wilcoxon signed-rank tests with
Holm correction are reported for $\lambda$ differences. All retained
evaluations share the same image-level splits, YOLO detections, and
evaluation codepath.
