...
\textbf{Polarity alignment across tracks.}
The two experimental tracks inherit opposite native label conventions: the CIFAR-10 OOD track encodes $y = 1$ as the OOD-proxy class (OOD-positive), while the public FTI\_Zer0P inkjet dataset's raw \texttt{metadata.csv} encodes $\texttt{label} = 1$ as GOOD and $\texttt{label} = 0$ as BAD (ID-positive). To keep the OOD-positive scoring convention stated in Section~\ref{sec:bg:ood:problem} consistent across both tracks, the inkjet pipeline inverts the raw label once at the dataset boundary, $\texttt{quality} = 1 - \texttt{label}$, so that the model is trained with $c = 0$ for GOOD (ID) and $c = 1$ for BAD (OOD-proxy), matching the CIFAR track. The inkjet anomaly score is therefore $s_{\text{QC}}(x) = e_g - e_b = e_{c=0} - e_{c=1}$, with higher values indicating \emph{more anomalous} (BAD-like) crops, and AUROC is reported with BAD as the positive class. With this alignment every AUROC and FPR95 reported in this thesis follows the same direction (higher score $\Rightarrow$ stronger evidence of being out-of-distribution or defective), independent of the underlying dataset's label dictionary.
