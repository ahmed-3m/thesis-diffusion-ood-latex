...
The two experimental tracks inherit opposite native label conventions: the CIFAR-10 OOD track encodes $y = 1$ as the OOD-proxy class (OOD-positive), while the public FTI\_Zer0P inkjet dataset encodes $\text{label} = 1$ as GOOD (ID-positive). To keep the OOD-positive scoring convention stated in Section~\ref{sec:bg:ood:problem} consistent across both tracks, the inkjet anomaly score is defined as $s_{\text{QC}}(x) = e_g - e_b$, so that higher scores indicate \emph{more anomalous} (BAD-like) crops, and AUROC for the inkjet track is reported with BAD as the positive class. With this alignment, every AUROC and FPR95 reported in this thesis follows the same direction (higher score $\Rightarrow$ stronger evidence of being out-of-distribution or defective), independent of the underlying dataset's label dictionary.
