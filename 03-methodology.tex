...
\item \textbf{Out-of-distribution (OOD):} $x_{\text{test}} \sim P_{\text{ood}}(x)$, where $P_{\text{ood}}$ is either the within-CIFAR OOD proxy ($y = 1$ pool) or an unseen external dataset. In the external benchmark protocol, these external samples are evaluated against the full CIFAR-10 test reference pool because the binary CDM was trained on both conditional branches of CIFAR-10 (airplane as $c{=}0$, non-airplane as $c{=}1$). The core auditable external set comprises CIFAR-100, Places365, FashionMNIST, SVHN, and Textures; Food-101 and STL-10 are retained only as legacy traceability entries.
\end{itemize}

\textbf{Binary Generative Classification Framework:} We train a single conditional diffusion model to learn the two class-conditional distributions $p_\theta(x \mid c)$ for $c \in \{0, 1\}$. For OOD detection we exploit the observation that ID samples should be reconstructed accurately under the ID condition ($c = 0$) but poorly under the OOD condition ($c = 1$), and vice versa for OOD samples.
...
5:\quad $e_0 \leftarrow e_0 + \|\epsilon_k - \epsilon_\theta(x_{t_k}, t_k, c{=}0)\|^2$\\
6:\quad $e_1 \leftarrow e_1 + \|\epsilon_k - \epsilon_\theta(x_{t_k}, t_k, c{=}1)\|^2$\\
7:\ \textbf{end for}\\
8:\ $e_0 \leftarrow e_0 / K$;\ $e_1 \leftarrow e_1 / K$\\
9:\ $s(x) \leftarrow e_0 - e_1$ (positive: OOD; negative: ID)\\
...
\textbf{Complexity Analysis:} The cost of Algorithm~1 is $O(2K \cdot N)$ where $N$ is the cost of one forward pass. For $K = 50$ and the binary UNet (${\approx}68.8$M parameters), this requires about 21.6 minutes per 10{,}000 images on an NVIDIA GeForce RTX 2080 Ti, roughly $492\times$ the wall-clock cost of a ResNet-18 forward pass on the same hardware. Section~\ref{sec:results:ablations} shows that $K = 10$ reduces this to about 3.0 minutes per 10{,}000 images ($68.6\times$ ResNet-18 cost) while achieving 98.2\% AUROC, close to the 98.6\% at $K = 50$.
...
\textbf{False Positive Rate at 95\% TPR (FPR@TPR95):} This metric measures the fraction of ID samples incorrectly flagged as OOD when the threshold is set such that 95\% of OOD samples are correctly detected:

\begin{equation}
\label{eq:meth:fprtpr95}
\text{FPR@TPR95} = \text{FPR}(\tau^*) \quad\text{where}\quad \tau^* = \inf\!\left\{\tau : \text{TPR}(\tau) \geq 0.95\right\}
\end{equation}
...
Second, unlike VAEs that decode in a single step, diffusion models iteratively refine over $T$ steps, providing multiple opportunities to accumulate evidence about whether a sample fits the learnt distribution. Each denoising step $x_t \to x_{t-1}$ can be viewed as a gradient step toward high-density regions.
...
For test samples $x$ from $p(x \mid y = c)$, we expect $e_c(x) \approx \mathcal{L}_c$ (low). For OOD samples, there is a distribution mismatch, leading to $e_c(x) \gg \mathcal{L}_c$ (high).
...
\textbf{Observation 1} \emph{(Standard MSE decomposition; ID reconstruction error bound).} \textit{For an ID sample $x \in \mathcal{M}_c$ drawn from class $c$, the expected reconstruction error under the correct class condition satisfies:}
...
\textbf{Conjecture 2} \emph{(OOD reconstruction error lower bound, informal).} \textit{For an OOD sample $x_{\text{ood}}$ at manifold distance $d(x_{\text{ood}}, \mathcal{M}_c) = \inf_{x' \in \mathcal{M}_c} \|x_{\text{ood}} - x'\|$ from the nearest ID manifold, the expected reconstruction error satisfies:}
...
\begin{itemize}
    ...
    \item \textbf{Class Conditioning:} Learned class embeddings $e_y \in \mathbb{R}^{d_{\text{emb}}}$ added to timestep embeddings
...
e_c(x) = \frac{1}{K} \sum_{k=1}^{K} \left\| \epsilon_k - \epsilon_\theta\!\left( \sqrt{\bar{\alpha}_{t_k}}\, x + \sqrt{1 - \bar{\alpha}_{t_k}}\, \epsilon_k,\, t_k,\, c \right) \right\|^2
\end{equation}

where we sample $K$ random timesteps $t_k \sim \mathcal{U}[1, T]$ and noise vectors $\epsilon_k \sim \mathcal{N}(0, I)$ to obtain a robust estimate. The OOD score is then computed via Equation~\ref{eq:meth:sood}.
...
c = e_{\text{tmpl}} + e_{\text{feat}} + e_{\text{qual}} + e_{\text{bbox}} + e_{\text{time}}
\end{equation}

where $e_{\text{time}}$ is the standard sinusoidal timestep embedding. The combined conditioning vector $c$ is injected at each UNet resolution level.
...
\epsilon_\theta(x_t, t, \text{tmpl}, \text{feat}, \text{qual}, \text{bbox}) = \text{UNet}(x_t, c)
\end{equation}
...
where $s = 0.008$ is a small offset parameter. For the inkjet pipeline, we use a linear schedule ($\beta\colon 1 \times 10^{-4} \to 0.02$) with $T = 1000$ timesteps, following standard DDPM practice for higher-resolution inputs.
...
We evaluate using AUROC (probability that a random OOD sample scores higher than a random ID sample; 1.0~$=$~perfect, 0.5~$=$~random) and FPR@TPR95 (false positive rate at 95\% true positive rate; lower is better) as primary metrics, with AUPRC and Detection Error reported for completeness. Formal definitions are provided in Section~\ref{sec:meth:problem:metrics}.
