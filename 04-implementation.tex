...
OOD detection requires $K$ forward passes per image. On an NVIDIA GeForce RTX 2080 Ti, $K = 50$ takes 4861.1\,s per 10K images ($492\times$ ResNet-18 wall-clock cost); $K = 10$ takes 972.9\,s ($68.6\times$) while maintaining 98.2\% AUROC. Both timings use the seed-42 $\lambda{=}0.01$ checkpoint; $K = 1$ achieves 91.0\% AUROC but was not re-timed, so no wall-clock ratio is claimed. For the inkjet pipeline, $K = 50$ takes $\sim$0.5 seconds per feature crop.

\paragraph{Determinism and Logging}

All retained experiments use fixed random seeds with PyTorch deterministic mode enabled. Configuration files, git commit hashes, and environment metadata (Python/CUDA versions) are logged automatically. Public verification draws on the released repositories, weights, raw scores, configuration files, and command sequences detailed in Appendix~\ref{app:experimental-config}.

\paragraph{Multi-GPU Training}

The code supports PyTorch Lightning's DDP with automatic gradient synchronisation, but all reported experiments use a single GPU per run; multi-GPU support is an implementation capability rather than part of the experimental protocol.
